...
async def connection_worker(
    ...
    if is_https:
        ...
        ssl_ctx.options |= sslmod.OP_NO_TICKET
    ...
    try:
        while time.time() < deadline:
            # Connect or reconnect on first iteration or after failure
            if writer is None:
                try:
                    ...
                    errs["connect"] = errs.get("connect", 0) + 1
                    fail += 1
            ...
            t0 = time.time()
            req_bytes = build_requests(host, port, path, 1, cf_bypass, rand_ua)
            ...
            try:
                ...
                status = 0
                if headers_raw.startswith(b"HTTP/1.1 200"):
                    status = 200
                elif headers_raw.startswith(b"HTTP/1.1 "):
                    try:
                ...
                fail += 1
                # Connection broken — close and reconnect on next iteration
                try:
                    writer.close()
